"""Quran video renderer using a repository-local animated background library."""
...
LOCAL_BACKGROUND_DIR = Path("assets/background_videos")
CHANNEL_NAME = os.getenv("QURAN_CHANNEL_NAME", "التجارة مع الله").strip() or "التجارة مع الله"
VISUAL_ENGINE_VERSION = "local_background_library_6.0"
...
def verify_local_video(path: Path) -> bool:
...
def load_background_library() -> list[dict]:
    raw = load_json(VIDEO_LIBRARY, [])
    if not isinstance(raw, list):
        raise RuntimeError("data/background_videos.json must contain a list.")
    valid = []
    for item in raw:
        ...
        if not rel:
            continue
        path = Path(rel)
        if verify_local_video(path):
            valid.append({**item, "resolved_path": str(path)})
    if not valid:
        raise RuntimeError(
            "No valid local background videos were found in assets/background_videos/."
        )
    return valid


def choose_background(segment: dict) -> dict:
    library = load_background_library()
    memory = load_json(MEMORY_PATH, {})
    recent = set(memory.get("recent_backgrounds", [])[-3:])
...
def remember_background(item: dict, segment_id: str) -> None:
    ...
    memory.update(recent_backgrounds=recent[-4:], history=history[-100:])
...
def cover_frame(array: np.ndarray, width: int, height: int, t: float, duration: float) -> Image.Image:
    ...
    zoom = 1.035 + 0.02 * (0.5 + 0.5 * math.sin((t / max(duration, .1)) * math.pi))
    size = (
        max(width, int(image.width * ratio * zoom)),
        max(height, int(image.height * ratio * zoom)),
    )
    image = image.resize(size, Image.Resampling.LANCZOS)
    extra_x, extra_y = image.width - width, image.height - height
    x = int(extra_x * (0.50 + 0.08 * math.sin(t * 0.05)))
    y = int(extra_y * (0.48 + 0.05 * math.cos(t * 0.04)))
...
def build_lines(items: list[str], font_path: str, max_width: int, max_height: int):
    ...
    fallback = None
    for size in range(76, 27, -2):
        font = ImageFont.truetype(font_path, size)
        spacing = max(8, int(size * .16))
        ...
        if len(lines) <= 5 and len(lines) * line_height <= max_height:
            return fallback
    if fallback is None:
        raise RuntimeError("Ayah text layout could not be created.")
    return fallback
...
def ornamental_overlay(width: int, height: int) -> Image.Image:
    ...
    gold = (225, 193, 108, 230)
    draw.rectangle((0, 0, width, height), fill=(18, 0, 24, 54))
    draw.rounded_rectangle((34, 28, width - 34, height - 28), radius=30, outline=gold, width=4)
    draw.arc((width * .16, -height * .24, width * .84, height * .64), 180, 360, fill=gold, width=7)
    draw.line((width * .16, height * .20, width * .16, height * .68), fill=gold, width=4)
    draw.line((width * .84, height * .20, width * .84, height * .68), fill=gold, width=4)
    return layer


def text_renderer(segment: dict, audio: dict, font_path: str):
    panel = (150, 665, WIDTH - 150, HEIGHT - 70)
    ...
    def render(index: int, highlighted: int | None, reciter: str):
        layer = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 0))
        draw = ImageDraw.Draw(layer, "RGBA")
        title_rect = (WIDTH // 2 - 250, 55, WIDTH // 2 + 250, 140)
        draw.rounded_rectangle(title_rect, radius=26, fill=(38, 5, 40, 198), outline=(229, 194, 106, 235), width=3)
        draw.text((WIDTH // 2, 98), f"سورة {segment['surah']}", font=title_font, fill=(255, 243, 212, 255), anchor="mm", direction="rtl", language="ar")
        draw.rounded_rectangle(panel, radius=34, fill=(24, 3, 28, 202), outline=(229, 194, 106, 230), width=4)
        ayah = segment["ayahs"][index]
        draw.text((WIDTH // 2, panel[1] + 43), f"الآية {ayah['ayah']}", font=badge_font, fill=(236, 215, 164, 255), anchor="mm", direction="rtl", language="ar")
        font, spacing, lines, line_height = layouts[index]
        y = panel[1] + 88 + (panel_height - 150 - len(lines) * line_height) // 2
        for line in lines:
            ...
            for word_index, word, advance in line:
                ...
                draw.text(
                    ...
                    word,
                    font=font,
                    fill=(246, 202, 89, 255) if selected else (255, 250, 237, 255),
                    anchor="ra",
                    direction="rtl",
                    language="ar",
                    stroke_width=2,
                    stroke_fill=(30, 5, 20, 190),
                )
                x -= advance + spacing
            y += line_height
        draw.text((WIDTH // 2, panel[3] - 32), reciter, font=footer_font, fill=(237, 226, 205, 235), anchor="mm", direction="rtl", language="ar")
        draw.text((WIDTH - 56, HEIGHT - 38), CHANNEL_NAME, font=watermark_font, fill=(239, 211, 138, 190), anchor="rs", direction="rtl", language="ar")
        return layer
...
def save_metadata(segment: dict, seo: dict, audio: dict, video: Path, preview: Path, background: dict) -> None:
    ...
    manifest = {
        ...
        "background_theme": background["id"],
        "background_source": background.get("source", "repository-local"),
        "background_license": background.get("license", "user-supplied-or-cleared"),
...
def build_video(segment: dict, seo: dict) -> str:
    ...
    background_path = Path(background["resolved_path"])
    ...
    def frame(t: float):
        ...
        image.alpha_composite(Image.new("RGBA", (WIDTH, HEIGHT), (14, 0, 20, 68)))
